...
phpstan:
	php bin/phpstan clear-result-cache -q && php -d memory_limit=400M bin/phpstan

phpstan-runtime-reflection:
	php bin/phpstan clear-result-cache -q && php -d memory_limit=400M bin/phpstan analyse -c phpstan-runtime-reflection.neon

phpstan-result-cache:
	php -d memory_limit=400M bin/phpstan

phpstan-generate-baseline:
	php -d memory_limit=400M bin/phpstan --generate-baseline

phpstan-validate-stub-files:
	php bin/phpstan analyse -c conf/config.stubFiles.neon -l 8 tests/notAutoloaded/empty.php

phpstan-pro:
	php -d memory_limit=400M bin/phpstan --pro
